...
const loadData = async () => {
  ...
  if (res.code === 0) {
    ...
    if (!form.value.judegCase) {
      form.value.judegCase = [];
    }
  } else {
  ...
  if (!form.value.judegCase) {
    ...
    form.value.judegCase = JSON.parse(form.value.judegCase as any);
  }
  if (!form.value.tags) {
    form.value.tags = [];
  } else {
...
const handleDelete = (index: number) => {
  form.value.judegCase.splice(index, 1);
};
